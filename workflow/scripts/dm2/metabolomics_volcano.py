#!/usr/bin/env python3
"""
Volcano plot — metabolomics differential features for dm2 (G1 vs G0).

Standalone, one-off script (not wired into Snakemake) — run manually:
    python workflow/scripts/dm2/metabolomics_volcano.py

Color: up (red) / down (blue) / not significant (gray), FDR<0.05.
...
Output:
    results/dm2/metabolomics_volcano.png
"""
import numpy as np
import pandas as pd
...
OUT        = Path("results/dm2/metabolomics_volcano.png")
FDR_CUTOFF = 0.05
...
sig_up   = df[(df["adj.P.Val"] < FDR_CUTOFF) & (df["logFC"] > 0)]
sig_down = df[(df["adj.P.Val"] < FDR_CUTOFF) & (df["logFC"] < 0)]
ns       = df[df["adj.P.Val"] >= FDR_CUTOFF]
...
texts = [
    ax.text(row["logFC"], row["neglog10_fdr"], row["plot_name"], fontsize=7.5)
